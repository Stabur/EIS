...
?>
  ...
  <div class="posts-list">
    <article class="post">
    <h2 class="post-title" align="center">Задания:</h2>
...
if(isset($_POST['close'])){
    header("Location: ./tasks.php#$idtask");
}
...
{
echo "<option ";
if($row_dw['id_depart']==$departTh) echo "selected=\"selected\"";
...
{
echo "<option ";
if($row_st['id_st']=="$statusTh") echo "selected=\"selected\"";
...
$row_redact = null; 
  ...
  include "./inc/new_task.php";

} elseif(intval(isset($_GET['close']))){
  header('Location: ./tasks.php#'.$_GET['close']);
...
if ($result) {
  // Вывод всех заданий
  header("Refresh:60; url=./tasks.php");
  echo "<div align=\"center\"><form method=\"get\"><button class=\"main\" type=\"submit\" name=\"task\" value=\"new\">Создать задание</button></form></div>";
  while($row = $result->fetch(PDO::FETCH_ASSOC)) {
      ...
      $histr = "SELECT * FROM tasks_history WHERE id_task=$row[id_task] ORDER BY `datetime_th` DESC ";
      $histr_task = $pdo_conn->query($histr);
      $row_histr = $histr_task->fetch(PDO::FETCH_ASSOC);
      $idTh = $row_histr['id_task'];
      $departTh = $row_histr['depart_th'];
  ...
  echo "<div align=\"center\"><form method=\"get\"><button class=\"main\" type=\"submit\" name=\"task\" value=\"new\">Создать задание</button></div></form>";

} else {
  echo "<div align=\"center\"><form method=\"get\"><button class=\"main\" type=\"submit\" name=\"task\" value=\"new\">Создать задание</button></form></div><br />
  <h2 class=\"post-title\">Нет заданий</h2>";
}
}
